...
2. Download and install the CData JDBC Driver for {source}: [https://www.cdata.com/drivers/alloydb/download/jdbc](https://www.cdata.com/drivers/alloydb/download/jdbc/)
3. License the CData JDBC Driver:
    * Navigate to the `lib` folder in the installation directory, typically:
        * (Windows) `C:\Program Files\CData\CData JDBC Driver for AlloyDB\`
        * (Mac/Linux) `/Applications/CData JDBC Driver for AlloyDB/`
    * Run the command `java -jar cdata.jdbc.allowdb.jar --license`
    * Enter your name, email, and "TRIAL" (or your license key).
4. Configure your connection to the data source (Salesforce as an example):
    * Run the command `java -jar cdata.jdbc.allowdb.jar` to open the Connection String utility.
...
5. Create a `.prp` file for your JDBC connection (e.g. `alloydb.prp`) using the following properties and format:
    ...
    * **Tables** - leave blank to access all data, otherwise you can explicitly declare the tables you wish to create access for
      ```env
      Prefix=allowdb
      ServerName=CDataAllowDB
      ServerVersion=1.0
      DriverPath=PATH\TO\cdata.jdbc.allowdb.jar
      DriverClass=cdata.jdbc.allowdb.AllowDBDriver
      JdbcUrl=jdbc:allowdb:InitiateOAuth=GETANDREFRESH;
      Tables=
      ```
